...
Элемент **Excludes** выполняет битовую маску указанного свойства и заданное значение. 
...
|[Bitmask](bitmask.md) <br/> |Представляет шестнадцатеричную или целой и дробной маску, используемый во время операции ограничения [Excludes](excludes.md) . Если битовая маска, которая представляет это шестнадцатеричный номер, он должен стоять 0 x или 0 X. В противном случае считается десятичного числа.  <br/> |
...
**Excludes** решит значение **true** , если операция выполнена на следующих разрешается в 0: 
  
1. Побитовое значение для свойства
    
2. Битовая маска значение для свойства
    
**Excludes** может применяться только для свойства, которое имеет значение типа integer. Если тип свойства — это что-либо кроме целое число, код ошибки **ErrorUnsupportedPathForQuery** возвращается в ответе. 
  
Обратный операции можно выполнить путем вызова Not(Excludes).
