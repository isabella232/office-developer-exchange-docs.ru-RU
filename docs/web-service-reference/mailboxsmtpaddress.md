...
Элемент **MailboxSmtpAddress** представляет SMTP-адрес пользователя, которого правила папки «Входящие», чтобы извлечь или обновить; или, Дата истечения срока действия пароля нужно вернуть. 
...
|**Элемент**|**Описание**|
|:-----|:-----|
|[GetInboxRules](getinboxrules.md) <br/> |Определяет запрос на получение правил папки "Входящие" почтового ящика в хранилище сервера.  <br/> |
|[GetPasswordExpirationDate](getpasswordexpirationdate.md) <br/> |Определяет запрос на получение срок действия пароля учетной записи электронной почты.  <br/> |
...
Элемент **MailboxSmtpAddress** — это необязательный элемент. Если элемент **MailboxSmtpAddress** опущен, используется адрес пользователя, выполнившего вход. 
