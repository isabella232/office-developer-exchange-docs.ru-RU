...
Элемент **ProxyPartnerToken** используется HTTP прокси-сервера компьютера, на котором выполняется Microsoft Exchange Server 2010 с установленной ролью сервера клиентского доступа. Этот элемент не используется с помощью операций веб-служб Exchange (EWS). 
