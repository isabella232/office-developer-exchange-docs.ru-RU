...
description: Элемент SendingAs представляет адрес электронной почты, который пользователь пытается отправить.
...
Элемент **SendingAs** представляет адрес электронной почты, который пользователь пытается отправить. 
...
|[RoutingType (EmailAddress)](routingtype-emailaddress.md) <br/> |Определяет тип адреса для почтового ящика. По умолчанию значение SMTP. Этот элемент является необязательным.  <br/> |
