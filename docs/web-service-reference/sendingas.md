...
Элемент **SendingAs** представляет адрес электронной почты, который пользователь пытается отправить как. 
...
|[RoutingType (EmailAddress)](routingtype-emailaddress.md) <br/> |Определяет тип адреса для почтового ящика. Значение по умолчанию — SMTP. Этот элемент является необязательным.  <br/> |
|[MailboxType](mailboxtype.md) <br/> |Представляет тип почтового ящика, представленного для электронной почты пользователя. Этот элемент является необязательным.  <br/> |
