...
Элемент **RoomList** представляет адрес электронной почты, который определяет список конференц-залы. 
...
|**Элемент**|**Описание**|
|:-----|:-----|
|[Имя (EmailAddressType)](name-emailaddresstype.md) <br/> |Задает отображаемое имя список помещений. Этот элемент является необязательным.  <br/> |
|[EmailAddress (NonEmptyStringType)](emailaddress-nonemptystringtype.md) <br/> |Определяет адрес Simple Mail Transfer Protocol (SMTP) в список помещений. Этот элемент является необязательным.  <br/> |
...
|**Элемент**|**Описание**|
|:-----|:-----|
|[GetRooms](getrooms.md) <br/> |Корневой элемент в запросе для получения списка комнат в пределах определенного почтового ящика помещения.  <br/> |
