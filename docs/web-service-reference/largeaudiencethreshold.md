...
Элемент **LargeAudienceThreshold** представляет большая аудитория пороговое значение для клиента. 
...
Текстовое значение представляет собой целое число, представляющее аудитории пороговое значение, которое указывает, что сообщение будет более чем одному пользователю.
