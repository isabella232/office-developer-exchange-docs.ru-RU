...
Элемент **ForwardAllowed** указывает, включена ли переадресация по электронной почте. 
...
Текстовое значение **true** для элемента **ForwardAllowed** указывает, что переадресация по электронной почте. Значение **false** указывает, что переадресации не допускается. 
